...
**Fork说明：**添加了获取的数值通过mqtt发送到指定服务器订阅，方便通过mqtt订阅获取电费相关数据，需在.env文件中设置mqtt的服务器地址，用户名，密码。另外由于自己不使用Home Assistant，所以禁用了Home Assistant的相关API推送功能，如果需要使用，可以在.env文件中设置为True并设置好HA的token即可。
